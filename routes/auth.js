...
// POST /api/auth/login — works for email, mobile, username, OR staffId
router.post('/login', async (req, res) => {
  try {
    ...
    // Match against loginEmail OR mobile OR staffId — covers all login styles
    const query = isEmail
      ? { loginEmail: id.toLowerCase() }
      : {
          $or: [
            { loginEmail: id.toLowerCase() },
            { mobileNo: id },
